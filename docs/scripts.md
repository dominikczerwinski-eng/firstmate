...
| `fm-slack-support.sh`    | Poll Fenedo Slack support reports, queue safe cosmetic reports for `fenedo-os`, hold risky or ambiguous reports, and reply in a completed report's Slack thread |
| `fm-slack-support.py`    | Standard-library Slack Web API client and durable support-state engine behind `fm-slack-support.sh` |
